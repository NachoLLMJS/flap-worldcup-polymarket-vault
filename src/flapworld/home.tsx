...
function AboutPage({ setRoute }){
  ...
  const faqs = [
    { q:t2('Is FlapWorld custodial?','FlapWorld 托管资金吗？'), a:t2('No. Every bet is a transaction you sign from your own wallet on BSC. Funds live in the contract, never with us.','不。每笔下注都是你用自己的钱包在 BSC 上签署的交易。资金在合约里，绝不在我们这里。') },
  ...
  return (
    <main className="bg-ink-950 pt-16">
      ...
      <section className="relative overflow-hidden border-b border-white/8">
        ...
        <div className="relative mx-auto grid max-w-[1320px] items-center gap-10 px-5 py-20 sm:px-6 sm:py-28 lg:grid-cols-[1.25fr_1fr]">
          ...
          <Reveal delay={120} className="hidden justify-self-center lg:block">
            <img src="/brand-logo.png" alt="FlapWorld" className="w-[300px]" style={{ filter:'drop-shadow(0 16px 50px rgba(215,255,54,0.25))' }}/>
          </Reveal>
        </div>
      </section>
      ...
      <section className="bg-ink-950 py-20 sm:py-24">
        <div className="mx-auto max-w-[1320px] px-5 sm:px-6">
          <Reveal>
            <span className="text-xs font-bold uppercase tracking-[0.2em] text-acid">{t2('Why FlapWorld','为什么选 FlapWorld')}</span>
